...
func TestSearchN8nDocsTool_Execute_ValidQuery(t *testing.T) {
	...
	// Test with valid query - this may fail if docs directory doesn't exist in test environment
	result, err := tool.Execute(ctx, map[string]any{"query": "webhook"})
	
	// Either we get results or no matching documentation message
	if err != nil {
		// If we can't read the docs directory in test environment, that's ok
	...
	// Should contain some indication of results or no results found
...
`
	...
	// The webhook node file is categorized as "action" unless the filename contains "trigger"
