...
func (s *GenerateN8NWorkflowSkill) GenerateN8NWorkflowHandler(ctx context.Context, args map[string]any) (string, error) {
	...
	content += "1. **Install n8n-cli**: Follow the installation guide at https://github.com/edenreich/n8n-cli\n"
	content += "   - Quick install: `curl -sSLf https://raw.github.com/edenreich/n8n-cli/main/install.sh | sh`\n"
	content += "2. Copy the YAML configuration above\n"
	content += "3. Save it to a `.yaml` file (e.g., `my-workflow.yaml`)\n"
	content += "4. Use n8n-cli to sync: `n8n workflows sync --directory workflows/`\n"
	content += "5. Configure any required credentials in your n8n instance\n"
	content += "6. Test the workflow by triggering it according to your trigger type\n\n"
	content += "## Next Steps\n\n"
